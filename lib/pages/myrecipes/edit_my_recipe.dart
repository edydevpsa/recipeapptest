...
class EditMyRecipe extends StatefulWidget {
  ...
  EditMyRecipe({this.idRecipe, this.recipe, this.uid}); //constructor
...
class _EditMyRecipeState extends State<EditMyRecipe> {
  ...
  void initState() {
    setState(() {
      ...
      this._recipe = widget.recipe.image;
    });
    ...
    if (url == null) {
      ...
      setState(() {
        ...
        box = BoxDecoration(
          border: Border.all(width: 1.0, color: Colors.black),
          shape: BoxShape.circle,
          image: DecorationImage(image: AssetImage('assets/images/azucar.gif')),
        );
      });
    } else {
      print('descarga la imagen');
      _downloadFile(url, widget.recipe.image).then((value) {
        _image = value;
        setState(() {
          box = BoxDecoration(
            border: Border.all(width: 1.0, color: Colors.black),
    ...
    if (_validar()) {
      ...
      auth.currentUser().then((value) {
        ...
        if (_image != null) {
          ...
          Firestore.instance.collection('usuarios')
          .document(usuario)
          .collection('mycolrecipes').add({
            'name'  : _name,
            'image' : urlFoto,
            'recipe': _recipe,
          }).then((value) => Navigator.of(context).pop())
          .catchError((onError) => print('Error al editar el usuario en la bd'));
          _isInAsyncCall = false;
        }
      }).catchError((onError) => _isInAsyncCall = false);
    } else {
      print('on¡bjeto no valido');
    }
  }
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Text('Doble click para cambiar la imagen'),
                ...
                TextFormField(
                  ...
                  validator: (value) => value.isEmpty ? 'El campo recipe esta vacio' : null,
